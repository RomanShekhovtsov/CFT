...
def train_lightgbm(X: pd.DataFrame, y: pd.Series, config: Config):
    ...
    cv = False  # len(X) < MAX_SAMPLE / 2
    hyperparams = hyperopt_lightgbm(X_sample, y_sample, params, config, cv)
...
@timeit
def hyperopt_lightgbm(X: pd.DataFrame, y: pd.Series, params: Dict, config: Config, cv: bool):
    ...
    def objective(hyperparams):
        if cv:
            eval_hist = lgb.cv({**params, **hyperparams}, full_data, 300, nfold = 10,
                          early_stopping_rounds=100, verbose_eval=100, stratified=config.is_classification())
            if config.is_classification():
                score = np.min(eval_hist['auc-mean'])
            else:
                score = np.min(eval_hist['rmse-mean'])
        else:
...
def data_sample(X: pd.DataFrame, y: pd.Series, nrows: int=MAX_SAMPLE) -> (pd.DataFrame, pd.Series):
    if len(X) > nrows:
